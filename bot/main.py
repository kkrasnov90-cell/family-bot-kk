...
from telegram.constants import ParseMode # Добавлено для явного использования Markdown
...
class FamilyBot:
    ...
    # 🎯 НОВАЯ ФУНКЦИЯ: Проверяет права администратора
    ...
    async def set_commands(self):
        ...
        commands = [
            ...
            ("file_id", "🔑 Получить ID файла (админ)"), # 🎯 ДОБАВЛЕНО
        ]
    ...
    async def start(self, update, context):
        ...
        Отправляет красивое приветствие, цели и возможности бота.
        """
        
        # 🎯 Получаем ID приветственной фотографии из config.py (если установлен)
        GREETING_PHOTO_ID = getattr(Config, 'GREETING_PHOTO_ID', None)
        
        message_text = (
            "**👋 Привет, я твой Семейный Хранитель!**\n\n"
            
            "Я создан, чтобы сделать семейную жизнь организованнее и теплее.\n\n"
            
            "### 🌟 Мои Главные Цели\n"
            "* **Ничего не забыть!** — Ежедневные уведомления о днях рождениях, годовщинах и других важных датах.\n"
            "* **Визуализация** — Использование фотографий для красивых и персонализированных напоминаний.\n"
            "* **Центр памяти** — Хранение всех важных данных о семье в одном месте, доступном только для тебя.\n\n"
            
            "### 🚀 Что я умею (без команд)\n"
            "Я работаю автоматически! Каждый день в **9:00 UTC** я проверяю базу и отправляю в этот чат:\n"
            "• **Поздравления с Днём Рождения** 🎂\n"
            "• **Напоминания о Годовщинах** 💍\n"
            "• **Информация о других важных событиях** 🗓️\n\n"
            
            "_Все команды для управления данными (добавление, удаление, списки) находятся в **Меню** (кнопка `/` рядом с полем ввода)._"
        )

        if GREETING_PHOTO_ID:
            # Отправляем приветственное фото с текстом в качестве подписи
            await update.message.reply_photo(
                photo=GREETING_PHOTO_ID,
                caption=message_text,
                parse_mode=ParseMode.MARKDOWN # Используем ParseMode.MARKDOWN
            )
        else:
            # Если фото ID не найден, отправляем только текст
    ...
    def run(self):
        ...
        # Использование asyncio.run_until_complete() для асинхронного вызова
        loop = asyncio.get_event_loop()
        # ВАЖНО: При необходимости очистки вебхука, выполните ее здесь
        # loop.run_until_complete(self.application.bot.delete_webhook())
